...
The lapel microphone is one of the most difficult inputs to manage on our sound desk and requires fine adjustments to get the best quality sound from it. Following our training session, I put some of my own advice into practice on this date to get the best sound I could manage, and I think the results will show that the settings were quite affective.
...
I usually. recommend a HPF set to around 180Hz on a vocal mic, but looking at the image below, I was a little more invasive with the HPF for this recording. The primary reason for this was to remove as much of the plosive sound as possible.

![HPF with altered slope](images/hpf.png)

The image above shows that I have also adjusted the slope of the cut on the filter. The default is 12db Butterworth, but I chose the 18db to increase the slope, making it steeper, therefore cutting more the the LF.

## PEQ

> {{< collapse summary="What is PEQ?" >}}

The Parametric Equaliser is our primary tool to adjust the tone of the selected input or output. By default, our sound desk is configured to display a real time analyser (RTA) of the audio of the selected channel, over the PEQ graph. This helps us manage the tone as we can see any peaks and troughs in the audio that may need to be adjusted
{{</ collapse >}}

As previously mentioned, Nolan does have a very clean sounding tone to his voice so not much work was needed. He does also generally speak quite loudly so feedback isn't usually a concern as it might be with someone with a quiet voice.
...
As always, good microphone placement is essential. The mic is required to be central to the body, not to far down the chest and not too far up. Probably the best location is between the mouth and sternum.

## Compression

> {{< collapse summary="What is compression?" >}}
> Compression is a tool used to affect control of the dynamic range of the signal. This means reducing the gap between the loudest moments, and the quietest moments. Usually this results in the overall signal being quieter, so we then apply an amount of gain to compensate for the reduction in dynamic range.
> {{</ collapse >}}

The compression applied to the mic on this occasion was most essential due to the aforementioned dynamics native to Nolan's style. The result was a natural sounding output from the PA System whilst maintaining suitable levels that when Nolan "whispered", he was still audible, and when he was loud, people weren't having to cover their ears.
...
**Threshold** is the important setting that can only be set once the gain and EQ have been correctly applied for the most accurate result. I would suggest the ideal setting for the threshold is such that the compressor is just "tickling" in during normal levels of speech, and barely active at all when Nolan was "whispering".

Finally, I added some gain after the compressor. When the compressor is working, it is reducing the dynamic range of the signal by reducing the loudest parts of the signal. This makes our audio all in all quieter. To compensate, we add some gain after the compressor, bringing our quiet sections up to an appropriate level, but keeping the range of the dynamics the same.

The histogram in the image above shows a moment where Nolan had been relatively loud and the compressor had kicked in quite a lot to compensate. When this happened, a keen listener would have heard Nolan more from his own voice as opposed to hearing him from the PA system, but the overall level in the room would have been similar. The unmastered recording will likely be quite noticeable and sound unnatural when listened back to.
